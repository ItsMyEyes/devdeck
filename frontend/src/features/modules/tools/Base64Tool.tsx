    ...
    <ToolCard
      ...
      actions={
        <>
          ...
          />
          <label className="flex items-center gap-1.5 font-mono text-[11px] text-loom-muted select-none">
            <input type="checkbox" checked={urlSafe} onChange={(e) => setUrlSafe(e.target.checked)} className="accent-loom-accent" />
            URL-safe
          </label>
        </>
      }
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-[1fr_auto_1fr]">
        ...
        <div className="flex items-center justify-center lg:pt-6">
          <button
            ...
            className="cursor-pointer rounded-md border border-loom-border-menu p-1.5 text-loom-muted-2 hover:text-loom-accent-soft"
          >
        ...
        <div className="flex min-h-[200px] flex-col gap-1.5">
          ...
          {error ? (
            <div className="flex min-h-[200px] flex-1 items-start gap-2 rounded-lg border border-loom-red-tint bg-loom-red-tint/40 p-3">
              <AlertTriangle size={14} className="mt-0.5 flex-none text-loom-red-soft" />
              <div className="font-mono text-[11px] text-loom-muted">{error}</div>
            </div>
          ) : (
            <pre className="min-h-[200px] flex-1 overflow-auto rounded-lg border border-loom-border-card bg-loom-terminal p-3 font-mono text-[11.5px] leading-relaxed whitespace-pre-wrap">
              {output}
            </pre>
          )}
        </div>
