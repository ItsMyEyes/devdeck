...
describe('ChatComposer — t3code layout', () => {
  it('renders a circular icon send button, not a labelled one', () => {
    render(<ChatComposer status="idle" onSend={vi.fn()} onAbort={vi.fn()} />)
  ...
  it('becomes the interrupt control while the thread is running', async () => {
    const onAbort = vi.fn()
    const onSend = vi.fn()
    render(<ChatComposer status="running" onSend={onSend} onAbort={onAbort} />)
  ...
  it('sends on Enter and inserts a newline on Shift+Enter', () => {
    const onSend = vi.fn()
    render(<ChatComposer status="idle" onSend={onSend} onAbort={vi.fn()} />)
  ...
  it('renders the status strip below the composer', () => {
    render(<ChatComposer status="idle" onSend={vi.fn()} onAbort={vi.fn()} worktree="devdeck" branch="main" />)
