// Pure logic for the composer's pending `AskUserQuestion` flow. Ported from
// t3code's `apps/web/src/pendingUserInput.ts`, verbatim in behavior — the
// only change is that `UserInputQuestion` is defined locally instead of
// imported from `@t3tools/contracts` (see plan T4 / spec §1.6). Answer ids
// are normalized once, in Go (never here) — see the plan's Global
// Constraints. No React, no DOM.
