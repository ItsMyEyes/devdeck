    ...
    <div>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="text-[9.5px] font-semibold uppercase tracking-[0.14em] text-devdeck-dim-2">Build</div>
          <div className="mt-1 text-[13px] font-semibold text-devdeck-fg">Version</div>
          <p className="mt-1 text-[11.5px] text-devdeck-muted-2">
            Build identity and update status for this device&apos;s local runtime.
          </p>
        </div>
        <div className="flex-none">
          {check.data?.managed ? (
            <p className="max-w-[220px] text-right font-mono text-[10.5px] text-devdeck-dim-2">
      ...
      {check.data ? (
        <p className="mt-2.5 font-mono text-[10.5px] text-devdeck-dim-2">
          {check.data.error
            ? check.data.error
      ...
      {check.isError ? (
        <p className="mt-2.5 font-mono text-[10.5px] text-devdeck-red-soft">
      ...
      <div className="rounded-lg border border-devdeck-border bg-devdeck-terminal p-3.5">
        {build.isLoading ? (
          ...
          <>
            ...
            <div className="mt-3 border-t border-devdeck-border pt-3">
              <div className="flex items-center justify-between gap-3">
                <span className="flex-none text-[9.5px] font-semibold uppercase tracking-[0.14em] text-devdeck-dim-2">
                  SHA256
                </span>
                {build.data.sha256 ? (
                  <div className="flex min-w-0 items-center gap-2">
                    <span className="min-w-0 truncate font-mono text-[10.5px] text-devdeck-dim-2">
                  ...
                  <span className="font-mono text-[10.5px] text-devdeck-dim-2">sha256 unavailable</span>
                )}
              </div>
            </div>
          </>
        ...
        {check.data?.checksumVerified === 'match' ? (
          <div className="mt-3 border-t border-devdeck-border pt-3">
            <p className="font-mono text-[10.5px] text-devdeck-green-soft">
        ...
        {check.data?.checksumVerified === 'mismatch' ? (
          <div className="mt-3 border-t border-devdeck-border pt-3">
            <p className="inline-flex items-center gap-1.5 font-mono text-[10.5px] text-devdeck-red-soft">
              <ShieldAlert size={12} />
