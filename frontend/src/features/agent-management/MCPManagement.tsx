  ...
  if (agents.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center p-4 text-center">
        <div className="flex h-[55vh] w-full max-w-[760px] flex-col items-center justify-center rounded-xl border border-dashed border-loom-border-strong">
          <ServerCog size={28} strokeWidth={1.5} className="text-loom-dim-2" />
          <div className="mt-3 text-[13px] font-medium text-loom-muted">
            No MCP-compatible agent is installed
          </div>
          <div className="mt-1.5 max-w-[46ch] text-[11.5px] leading-relaxed text-loom-dim">
            Install Claude Code or Codex to manage native MCP servers from Loom.
          </div>
        </div>
      </div>
    )
    ...
    <div className="flex flex-none flex-col md:min-h-0 md:flex-1">
      <div className="grid flex-none grid-cols-2 border-b border-loom-border bg-loom-surface/20 md:grid-cols-[minmax(220px,1.2fr)_minmax(150px,0.7fr)_minmax(150px,0.7fr)]">
        ...
        <div className="border-r border-loom-border px-3 py-3 sm:px-4">
          ...
          <div className="mt-1 font-mono text-[19px] font-semibold text-loom-fg">{uniqueServers}</div>
        </div>
        <div className="px-3 py-3 sm:px-4">
          <div className="font-mono text-[9.5px] uppercase tracking-[0.12em] text-loom-dim">
            Active configs
          </div>
          <div className="mt-1 font-mono text-[19px] font-semibold text-loom-green-soft">
            {connected}
          </div>
        </div>
      </div>

      <div className="flex flex-none flex-col gap-2 border-b border-loom-border px-3 py-3 sm:flex-row sm:items-center sm:px-4">
      ...
      <div className="flex-none overflow-visible p-3 sm:p-4 md:min-h-0 md:flex-1 md:overflow-auto">
        {loading && servers.length === 0 ? (
          ...
          <div className="flex min-h-[280px] flex-col items-center justify-center rounded-xl border border-dashed border-loom-border-strong text-center">
            ...
            <div className="mt-3 text-[12.5px] font-medium text-loom-muted">
          ...
          <div className="flex min-h-[340px] flex-col items-center justify-center rounded-xl border border-dashed border-loom-border-strong text-center">
            <ServerCog size={27} strokeWidth={1.5} className="text-loom-dim-2" />
            <div className="mt-3 text-[13px] font-medium text-loom-muted">No MCP servers configured</div>
            <div className="mt-1.5 max-w-[46ch] text-[11.5px] leading-relaxed text-loom-dim">
              Add a local command or remote HTTP server and Loom will write it through the selected
              agent CLI.
            </div>
          ...
          <div className="mx-auto w-full max-w-[1180px] overflow-hidden rounded-xl border border-loom-border-card bg-loom-card">
            <div className="hidden grid-cols-[minmax(200px,1fr)_110px_120px_96px_36px] gap-3 border-b border-loom-border px-3.5 py-2 font-mono text-[9.5px] uppercase tracking-[0.1em] text-loom-dim md:grid">
              <span>Server</span>
              <span>Agent</span>
              <span>Transport</span>
              <span>Status</span>
              ...
              return (
                <article
                  ...
                  className={cn(
                    ...
                    index > 0 && 'border-t border-loom-border',
                  )}
                >
                  <div className="col-span-2 flex min-w-0 items-start gap-3 md:col-span-1">
                    ...
                    <div className="min-w-0">
                      <div className="truncate font-mono text-[12px] font-semibold text-loom-fg">
                        {server.name}
                      </div>
                      <div className="mt-1 flex min-w-0 items-center gap-2">
                        <span className="truncate font-mono text-[9.5px] text-loom-dim">
                          {server.target || 'Target hidden by agent'}
                        </span>
                        {server.envKeys.length > 0 ? (
                          <span
                            title={server.envKeys.join(', ')}
                            className="inline-flex flex-none items-center gap-1 text-[9.5px] text-loom-muted-2"
                          >
                            <KeyRound size={9} />
                  ...
                  <div className="flex items-center gap-2">
                    <AgentMark
                      ...
                      active
                    />
                    <span className="text-[10.5px] text-loom-muted">
                      {agent?.name ?? server.agentId}
                    </span>
                  </div>

                  <span className="w-fit rounded-md border border-loom-border-card bg-loom-surface-2 px-2 py-1 font-mono text-[9.5px] uppercase text-loom-muted-2">
                    {server.transport}
                  ...
                  <button
                    ...
                    className="flex h-9 w-9 cursor-pointer items-center justify-center justify-self-end rounded-lg text-loom-dim transition-colors hover:bg-loom-red-tint-hover hover:text-loom-red-soft focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 md:h-8 md:w-8"
                  >
                    <Trash2 size={13} />
                  </button>
                </article>
...
function statusClass(status: string, enabled: boolean) {
  ...
  return cn(
    ...
    healthy
      ? 'border-loom-green-tint-border bg-loom-green-tint text-loom-green-soft'
      : 'border-loom-red-tint bg-loom-red-tint-hover text-loom-red-soft',
  )
}
...
function MCPListSkeleton() {
  return (
    <div
      className="mx-auto w-full max-w-[1180px] overflow-hidden rounded-xl border border-loom-border-card bg-loom-card"
      ...
      {[0, 1, 2].map((item) => (
        <div
          ...
          className={cn(
            ...
            item > 0 && 'border-t border-loom-border',
          )}
        >
          <div className="h-8 rounded-md bg-loom-elevated" />
          <div className="h-7 rounded-md bg-loom-surface-2" />
          <div className="h-7 rounded-md bg-loom-surface-2" />
          <div className="h-7 rounded-md bg-loom-surface-2" />
          <div className="h-7 w-7 rounded-md bg-loom-surface-2" />
        </div>
      ))}
    </div>
  )
