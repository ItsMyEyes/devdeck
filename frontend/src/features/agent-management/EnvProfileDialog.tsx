  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 flex-none items-center justify-center rounded-xl border border-devdeck-line bg-devdeck-on text-devdeck-fg-2">
          <KeyRound size={18} />
        </div>
        <div>
      ...
      <div className="mt-6 grid gap-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="grid gap-1.5">
            <span className="text-[11.5px] font-medium text-devdeck-fg-2">Name</span>
            <Input
              value={name}
          ...
          <label className="grid gap-1.5">
            <span className="text-[11.5px] font-medium text-devdeck-fg-2">Base URL</span>
            <Input
              value={baseUrl}
        ...
        {/* auth token */}
        <label className="grid gap-1.5">
          <span className="text-[11.5px] font-medium text-devdeck-fg-2">
          ...
          <Input
            ...
            className="h-9 text-[12.5px] font-mono"
          />
          <span className="font-mono text-[9.5px] text-devdeck-fg-2">
            {isEdit
        ...
        {isCodex ? (
          <fieldset className="rounded-xl border border-devdeck-border-card bg-devdeck-card-wash p-4">
            <legend className="mb-3 text-[11.5px] font-medium text-devdeck-fg-2">
              Provider config
            </legend>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1.5">
                <span className="font-mono text-[9px] uppercase tracking-[0.1em] text-devdeck-fg-2">
                  Provider name
                </span>
                <Input
              ...
              <label className="grid gap-1.5">
                ...
                <select
                  ...
                  className="h-8 rounded-lg border border-devdeck-border-card bg-devdeck-pane px-2 text-[11px] font-mono text-devdeck-fg focus:outline-none focus:ring-2 focus:ring-ring/50"
                >
          ...
          <>
            ...
            <fieldset className="rounded-xl border border-devdeck-border-card bg-devdeck-card-wash p-4">
              <div className="mb-3 flex items-center justify-between">
                <legend className="text-[11.5px] font-medium text-devdeck-fg-2">Model slots</legend>
                <button
                  type="button"
                  onClick={() => void runFetchModels()}
                  disabled={fetchModels.isPending}
                  className="inline-flex h-7 cursor-pointer items-center gap-1.5 rounded-lg border border-devdeck-border-card bg-devdeck-pane px-2.5 font-mono text-[10px] text-devdeck-fg-2 transition-colors hover:border-devdeck-border-strong hover:text-devdeck-fg disabled:opacity-55"
                >
              ...
              {fetched.length > 0 ? (
                <p className="mt-2 text-[9.5px] text-devdeck-fg-2">
            ...
            <div className="rounded-xl border border-devdeck-border-card px-4 py-3">
              <div className="mb-2 flex items-center gap-2 text-[11px] font-medium text-devdeck-fg-2">
                <LockKeyhole size={12} className="text-devdeck-fg-2" />
              ...
              <div className="flex flex-wrap gap-1.5">
                {DEFAULT_ENV_KEYS.map((key) => (
                  <span
                    key={key}
                    className="rounded-md border border-devdeck-border-card bg-devdeck-card-wash px-1.5 py-1 font-mono text-[9px] leading-none text-devdeck-fg-2"
                  >
                    {key}
                  </span>
                ))}
        ...
        {/* ── custom env keys ── */}
        <fieldset>
          <legend className="mb-2.5 text-[11.5px] font-medium text-devdeck-fg-2">Custom env keys</legend>
          <div className="grid gap-2">
            {rows.length === 0 ? (
              <div className="rounded-lg border border-dashed border-devdeck-border px-3 py-3 text-center text-[11px] text-devdeck-fg-2">
                No custom keys yet.
              </div>
            ) : (
              rows.map((row, index) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="flex h-8 w-8 cursor-pointer items-center justify-center rounded-lg text-devdeck-fg-2 transition-colors hover:bg-devdeck-red-tint-hover hover:text-devdeck-err"
                  >
                    <Trash2 size={13} />
                  </button>
                </div>
          ...
          <button
            type="button"
            onClick={addRow}
            className="mt-2.5 inline-flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border border-devdeck-border-card bg-devdeck-card-wash px-2.5 text-[11px] text-devdeck-fg-2 transition-colors hover:border-devdeck-border-strong hover:text-devdeck-fg"
          >
            <Plus size={12} />
            Add key
          </button>
        ...
        {error ? (
          <div
            role="alert"
            className="rounded-lg border border-devdeck-red-tint bg-devdeck-red-tint-hover px-3 py-2 text-[11px] text-devdeck-err"
          >
            {error}
          </div>
        ) : null}
